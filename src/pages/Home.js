...
const Home = () => {
  ...
  // Setup Authorization header with the token if available
  const authHeader = token ? { Authorization: `${token}` } : {};

  // Fetch user data with the token in the Authorization header
  const { data: userData, loading: userLoading, error: userError } = useQuery(GET_USERS_QUERY, {
    context: {
      headers: authHeader,  // Attach the Authorization header to the request
    },
  });

  // Fetch task data with the token in the Authorization header
  const { data: taskData, loading: taskLoading, error: taskError } = useQuery(GET_TASKS_QUERY, {
    context: {
      headers: authHeader,  // Attach the Authorization header to the request
    },
  ...
  if (userLoading || taskLoading) return <p className="text-center text-quaternary">Loading...</p>;

  if (userError) return <p className="text-center text-red-600">Error: {userError.message}</p>;

  if (taskError) return <p className="text-center text-red-600">Error: {taskError.message}</p>;
